# training scripts of Transference model
...
CUDA_VISIBLE_DEVICES=$devices python3 $nematus_home/nematus/train.py \
    --datasets $data_dir/train.lc.bpe.gp.$src \
               $data_dir/train.lc.bpe.gp.$mt \
               $data_dir/train.lc.bpe.gp.$tgt \
    --f_datasets $data_dir/fine_tune/train.lc.bpe.gp.$src \
                 $data_dir/fine_tune/train.lc.bpe.gp.$mt \
                 $data_dir/fine_tune/train.lc.bpe.gp.$tgt \
    --dictionaries $data_dir/train.lc.bpe.gp.$src.json \
    ...
    --valid_datasets $data_dir/dev.lc.bpe.gp.$src \
                     $data_dir/dev.lc.bpe.gp.$mt \
                     $data_dir/dev.lc.bpe.gp.$tgt \
    --valid_deixis_datasets $consis_dir/deixis_dev.lc.bpe.gp.$src \
                            $consis_dir/deixis_dev.lc.bpe.gp.$mt \
                            $consis_dir/deixis_dev.lc.bpe.gp.$tgt \
    --valid_cohesion_datasets $consis_dir/lex_cohesion_dev.lc.bpe.gp.$src \
                              $consis_dir/lex_cohesion_dev.lc.bpe.gp.$mt \
