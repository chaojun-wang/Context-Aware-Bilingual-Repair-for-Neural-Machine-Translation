...
def load_data(config):
    ...
    text_iterator = TextIterator(
                        ...
                        is_train=True,
                        beam_mode_pro=config.beam_mode_pro,
                        noise_mode_pro=config.noise_mode_pro,
                        sample_mode_pro=config.sample_mode_pro,
                        beam_dropout=config.beam_dropout,
                        noise_dropout=config.noise_dropout,
                        sample_dropout=config.sample_dropout,
    ...
    if config.valid_freq and config.valid_deixis_source_dataset and config.valid_deixis_target_dataset:
        ...
        logging.info('no deixis consistency validation set loaded')
        valid_deixis_text_iterator = None
    ...
    if config.valid_freq and config.valid_cohesion_source_dataset and config.valid_cohesion_target_dataset:
        ...
        logging.info('no cohesion consistency validation set loaded')
        valid_cohesion_text_iterator = None
    logging.info('Done')
    return text_iterator, valid_text_iterator, valid_deixis_text_iterator, valid_cohesion_text_iterator
...
def train(config, sess):
    ...
    # inference graph only for validation inference and sampling check during training
    if config.beam_freq or config.valid_script is not None:
        beam_search_sampler = BeamSearchSampler(
            models=[replicas[0]],
            configs=[config],
            beam_size=config.beam_size,
            conservative_penalty=config.conservative_penalty,
            conservative_way=config.conservative_way)

    #save model options
    write_config_to_json_file(config, config.saveto)

    text_iterator, valid_text_iterator, valid_deixis_text_iterator, valid_cohesion_text_iterator = load_data(config)
        ...
        for source_sents, mt_sents, target_sents in text_iterator:
            ...
            if config.valid_freq and progress.uidx % config.valid_freq == 0:
                ...
                if config.valid_consis_script is not None:
                    ...
                    # reset bad counter and save best checkpoint also according to accuracy of consistency test.
                    need_to_save_consis = (dex_score is not None and
                                    (len(progress.valid_dex_scores) == 0 or
                                     dex_score > max(progress.valid_dex_scores))) or\
                                   (lex_score is not None and
                                    (len(progress.valid_lex_scores) == 0 or
                                     lex_score > max(progress.valid_lex_scores)))
                    if dex_score is None:
                        dex_score = 0.0  # ensure a valid value is written
                    if lex_score is None:
                        lex_score = 0.0  # ensure a valid value is written
                    progress.valid_dex_scores.append(dex_score)
                    progress.valid_lex_scores.append(lex_score)
                else:
                    need_to_save_consis = False
                ...
                if config.valid_script is not None:
                    ...
                    if need_to_save or need_to_save_consis:
                        progress.bad_counter = 0
...
def validate(session, model, config, text_iterator):
    ce_vals, token_counts = calc_cross_entropy_per_sentence(
        session, model, config, text_iterator, label_smoothing=config.label_smoothing, normalization_alpha=0.0)
...
def validate_consis_with_script(session, model, config, deixis_text_iterator, cohesion_text_iterator):
    ...
    ce_vals, token_counts = calc_cross_entropy_per_sentence(
        session, model, config, deixis_text_iterator, label_smoothing=0, normalization_alpha=config.normalization_alpha)
    out.write('\n'.join([str(i) for i in ce_vals])+'\n')
    out.flush()
    args = [config.valid_consis_script, out.name, 'deixis_dev']
    ...
    dex_score = None
    if len(stderr) > 0:
        logging.info("Consistency validation script wrote the following to standard "
                     "error:\n" + stderr)
    if proc.returncode != 0:
        logging.warning("Consistency validation script failed (returned exit status of "
                        "{}).".format(proc.returncode))
    try:
        dex_score = float(stdout.split('Total accuracy: ')[1].split('\n')[0])
    except:
        logging.warning("Consistency validation script output does not look like a score: "
                        "{}".format(stdout))
    logging.info("deixis set accuracy: {}".format(dex_score))

    logging.info('lex_cohesion set')
    out = tempfile.NamedTemporaryFile(mode='w')
    ce_vals, token_counts = calc_cross_entropy_per_sentence(
        session, model, config, cohesion_text_iterator, label_smoothing=0, normalization_alpha=config.normalization_alpha)
    out.write('\n'.join([str(i) for i in ce_vals]) + '\n')
    out.flush()
    args = [config.valid_consis_script, out.name, 'lex_cohesion_dev']
    proc = subprocess.Popen(args, stdin=None, stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE)
    stdout_bytes, stderr_bytes = proc.communicate()
    encoding = locale.getpreferredencoding()
    ...
    if proc.returncode != 0:
        logging.warning("Consistency validation script failed (returned exit status of "
                        "{}).".format(proc.returncode))
    try:
        lex_score = float(stdout.split('Total accuracy: ')[1].split('\n')[0])
    except:
        logging.warning("Consistency validation script output does not look like a score: "
                        "{}".format(stdout))
    logging.info("lex_cohesion set accuracy: {}".format(lex_score))

    return dex_score, lex_score
...
def calc_cross_entropy_per_sentence(session, model, config, text_iterator, label_smoothing,
    ...
    for xx, pp, yy in text_iterator:
        ...
        if normalization_alpha:
            adjusted_lens = [((5 + n) ** normalization_alpha) / ((5 + 1) ** normalization_alpha) for n in batch_token_counts]
