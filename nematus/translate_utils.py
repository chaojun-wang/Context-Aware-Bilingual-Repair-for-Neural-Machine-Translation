    ...
    for model, config in zip(sampler.models, sampler.configs):
        if config.model_type == 'rnn':
            ...
            if penalty_matrix is None:
                ...
                feed_dict[sampler.inputs.penalty_matrix] = penalty_matrix
...
def translate_file(input_file, input_p_file, output_file, session, sampler, config,
    ...
    def translate_maxibatch(maxibatch, target_to_num, num_to_source, num_to_target, num_prev_translated):
    ...
    _, target_to_num, num_to_source, num_to_target = util.load_dictionaries(config)
