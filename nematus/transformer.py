...
class Transformer(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope('{:s}_loss'.format(self.name)):
            ...
            if self.config.loss_function == 'MRT':
                ...
                if self.config.weighted_conservative_loss:
                    self._risk = mru.mrt_cost(self._loss_per_sentence, self.scores, self.index, self.config, self.mrt_penalty_weight)
                else:
    ...
    def _convert_inputs(self, inputs):
        ...
        # add <GO> token at the start of each sentence
