...
class BeamSearchSampler:
    ...
    def __init__(self, models, configs, beam_size, conservative_penalty, conservative_way):
        ...
        with tf.name_scope('beam_search'):
            ...
            sequences, scores = _beam_search(
                ...
                penalty_matrix=self.inputs.penalty_matrix,
                conservative_penalty=conservative_penalty,
                conservative_way=conservative_way,
...
def _beam_search(model_adapters, beam_size, batch_size_x, max_translation_len,
                 normalization_alpha, penalty_matrix, conservative_penalty, conservative_way, vocab_size, eos_id):
...
def _generate_while_loop_body_func(model_adapters, decoding_functions,
                                   ...
                                   vocab_size, eos_id, normalization_alpha, penalty_matrix,
                                   conservative_penalty, conservative_way):
    ...
    def extend_hypotheses(current_time_step, alive_sequences, alive_scores,
        ...
        for i in range(len(decoding_functions)):
            ...
            if conservative_penalty:
                if conservative_way == 'logits':
                    step_logits = step_logits - penalty_matrix
                    log_probs = tf.nn.log_softmax(step_logits, axis=-1)
                else:
                    log_probs = tf.nn.log_softmax(step_logits, axis=-1) - penalty_matrix
            else:
                log_probs = tf.nn.log_softmax(step_logits, axis=-1)
