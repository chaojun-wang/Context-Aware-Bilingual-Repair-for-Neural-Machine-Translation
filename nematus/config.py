...
class ConfigSpecification:
    ...
    def _define_param_specs(self):
        ...
        group.append(ParameterSpecification(
            ...
            visible_arg_names=['--f_source_dataset'],
        ...
        group.append(ParameterSpecification(
            ...
            visible_arg_names=['--f_target_dataset'],
            derivation_func=_derive_f_target_dataset,
            type=str, metavar='PATH',
            help='parallel training corpus for fine-tuning (target)'))
        ...
        group.append(ParameterSpecification(
            name='beam_mode_pro', default=0.0,
            visible_arg_names=['--beam_mode_pro'],
            type=float, metavar='FLOAT',
            help='Probability for selecting beam-searched round-trip translations as input (default: '
                 '%(default)s)'))

        group.append(ParameterSpecification(
            name='noise_mode_pro', default=0.0,
            visible_arg_names=['--noise_mode_pro'],
            type=float, metavar='FLOAT',
            help='Probability for selecting noisy synthetic translations as input (default: '
                 '%(default)s)'))

        group.append(ParameterSpecification(
            name='sample_mode_pro', default=1.0,
            visible_arg_names=['--sample_mode_pro'],
            type=float, metavar='FLOAT',
            help='Probability for selecting randomly-sampled round-trip translations as input (default: '
                 '%(default)s)'))

        group.append(ParameterSpecification(
            name='beam_dropout', default=0.1,
            visible_arg_names=['--beam_dropout'],
            type=float, metavar='FLOAT',
            help='Dropout probability of beam-searched sentence (default: '
                 '%(default)s)'))

        group.append(ParameterSpecification(
            name='noise_dropout', default=0.2,
            visible_arg_names=['--noise_dropout'],
            type=float, metavar='FLOAT',
            help='Dropout probability of tgt-side sentence (default: '
                 '%(default)s)'))

        group.append(ParameterSpecification(
            name='sample_dropout', default=0.1,
            visible_arg_names=['--sample_dropout'],
            type=float, metavar='FLOAT',
            help='Dropout probability of randomly-sampled sentence (default: '
                 '%(default)s)'))

        group.append(ParameterSpecification(
            name='noise_source', default=False,
            visible_arg_names=['--noise_source'],
            action='store_true',
            help='noise_source'))

        group.append(ParameterSpecification(
            name='extreme_noise', default=0.0,
            visible_arg_names=['--extreme_noise'],
            type=float, metavar='FLOAT',
            help='Probability of extreme noise source-side data (default: '
                 '%(default)s)'))

        group.append(ParameterSpecification(
            name='f_ratio', default=0.0,
            visible_arg_names=['--f_ratio'],
            type=float, metavar='FLOAT',
            help='the proportion of feeding fine-tuning data (default: '
                 '%(default)s)'))
        ...
        group.append(ParameterSpecification(
            name='chunk_size', default=10000000,
            visible_arg_names=['--chunk_size'],
            type=int, metavar='INT',
        ...
        group.append(ParameterSpecification(
            name='weighted_conservative_loss', default=False,
            visible_arg_names=['--weighted_conservative_loss'],
            action='store_true',
            help='weighting loss to enforces the system to be more conservative and learn fewer edits'))
        ...
        group.append(ParameterSpecification(
            name='valid_deixis_source_dataset', default=None,
            visible_arg_names=['--valid_deixis_source_dataset'],
        ...
        group.append(ParameterSpecification(
            ...
            visible_arg_names=['--valid_deixis_target_dataset'],
            derivation_func=_derive_valid_deixis_target_dataset,
            type=str, metavar='PATH',
            help='deixis consistency target-side validation corpus (default: %(default)s)'))
        ...
        group.append(ParameterSpecification(
            name='valid_cohesion_source_dataset', default=None,
            visible_arg_names=['--valid_cohesion_source_dataset'],
        ...
        group.append(ParameterSpecification(
            ...
            visible_arg_names=['--valid_cohesion_target_dataset'],
            derivation_func=_derive_valid_cohesion_target_dataset,
            type=str, metavar='PATH',
            help='cohesion consistency target-side validation corpus (default: %(default)s)'))
        ...
        group.append(ParameterSpecification(
            name='valid_consis_script', default=None,
            visible_arg_names=['--valid_consis_script'],
            type=str, metavar='PATH',
            help='path to script for external consistency test validation (default: '
                 '%(default)s).'))
        ...
        group.append(ParameterSpecification(
            name='conservative_penalty', default=0,
            visible_arg_names=['--conservative_penalty'],
            type=float, metavar='FLOAT',
            help='the conservative penalty parameters (c) (not for MRT sampling) (default: %(default)s)'))

        group.append(ParameterSpecification(
            name='conservative_way', default='probabilities',
            visible_arg_names=['--conservative_way'],
            type=str, choices=['probabilities', 'logits'],
            help='the place where conservative penalty assigns, either on '
                 'probabilities or logits (default: %(default)s)'))

        group.append(ParameterSpecification(
            name='add_eos', default=False,
            visible_arg_names=['--add_eos'],
            action='store_true',
            help='see <EOS> tokens as a existing token in the source/mt sentences'
                 'and do not give punishment to generate it'))
...
def _check_config_consistency(spec, config, set_by_user):
    ...
    if config.data_mode == 'multiple':
        if config.beam_mode_pro + config.noise_mode_pro + config.sample_mode_pro != 1:
            msg = 'beam, sample, noisy as a probability, the sum of these three probability' \
                  'must equal to one'
            error_messages.append(msg)
...
def _derive_f_source_dataset(config, meta_config):
    if config.f_source_dataset is not None:
        return config.f_source_dataset
...
def _derive_valid_deixis_source_dataset(config, meta_config):
    if config.valid_deixis_source_dataset is not None:
        return config.valid_deixis_source_dataset
...
def _derive_valid_cohesion_source_dataset(config, meta_config):
    if config.valid_cohesion_source_dataset is not None:
        return config.valid_cohesion_source_dataset
