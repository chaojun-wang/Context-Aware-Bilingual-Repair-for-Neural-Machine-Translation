...
class TranslationSettings(BaseSettings):
    ...
    def _add_console_arguments(self):
        ...
        self._parser.add_argument(
            '-cp', '--conservative_penalty', type=float, default=0.0,
            help="the conservative penalty parameters (c)")

        self._parser.add_argument(
            '-cw', '--conservative_way', type=str, choices=['probabilities', 'logits'], default="probabilities",
            help="the place where conservative penalty assigns, either on probabilities or logits")

        self._parser.add_argument(
            '-ae', '--add_eos', default=False, action='store_true',
