...
class ModelUpdater(object):
    ...
    def update(self, session, x, x_mask, y, y_mask, num_to_target, write_summary,
        ...
        for i in range(0, len(split_x), len(self._replicas)):
            ...
            for j in range(len(self._replicas)):
                ...
                if self._config.weighted_conservative_loss:
    ...
    def _split_and_pad_minibatch(self, x, x_mask, y, y_mask, penalty_weight, start_points):
        ...
        if penalty_weight is not None:
            split_penalty_weight = split_array(penalty_weight, start_points)

        # split_penalty_weight [[[xx,xx]],[[xx,xx]],...,[[xx,xx]]]
        ...
        if penalty_weight is not None:
            pad(split_penalty_weight, padding_size)
            penalty_weight = split_penalty_weight
    ...
    def _split_and_pad_minibatch_mrt(self, x, x_mask, y, y_mask, penalty_weight, score, start_points, index):
        ...
        # pad function sepecifcally for MRT index
        # index must be with type: list (1, num_real_sent)
        # eg: index: [[0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40]]
        def pad_index(split_a, padding_size):
            assert len(split_a) > 0
            dummy_array = numpy.array([0,1])
            for i in range(padding_size):
                split_a.append(dummy_array)
...
class _ModelUpdateGraph(object):
    ...
    def __init__(self, config, num_gpus, replicas, optimizer, global_step):
        ...
        for i in range(len(self._replicas)):
            name = 'penalty_weight_{}'.format(i)
            placeholder = tf.placeholder(name=name, shape=(None, ), dtype=tf.float32)
            self._penalty_weight.append(placeholder)
            ...
            with tf.device(device_spec):
                with tf.variable_scope(tf.get_variable_scope(), reuse=(i>0)):
                    if self._config.print_per_token_pro:
                        ...
                        if self._config.weighted_conservative_loss:
                            ce_per_sent = self._replicas[i].loss_per_sentence * self._penalty_weight[i]
                        else:
